...
export const SubscriptionPlans: React.FC<SubscriptionPlansProps> = ({
  ...
  const getFeatureList = (plan: Plan) => {
    ...
    if (
      typeof plan.features.workouts === "number" &&
      plan.features.workouts === -1
    ) {
      features.push("Unlimited workout logging");
    } else if (typeof plan.features.workouts === "number") {
      features.push(`${plan.features.workouts} workouts/month`);
    }

    if (
      typeof plan.features.ai_recommendations === "number" &&
      plan.features.ai_recommendations === -1
    ) {
      features.push("Unlimited AI recommendations");
    } else if (
      typeof plan.features.ai_recommendations === "number" &&
      plan.features.ai_recommendations > 0
    ) {
      features.push(
        `${plan.features.ai_recommendations} AI recommendations/month`
      );
    }
    ...
    <div className="space-y-8">
                ...
                <div className="text-center mb-6">
                  ...
                  {plan.interval_type === "year" && plan.price > 0 && (
                    <p className="text-gray-500 text-sm mt-1">
                      {monthlyPrice.toFixed(2)} EGP/month billed annually
                    </p>
                  )}
                </div>
      ...
      {selectedPlan && (
        <PaymentModal
          ...
          billingCycle={
            selectedPlan.interval_type === "year" ? "yearly" : "monthly"
          }
